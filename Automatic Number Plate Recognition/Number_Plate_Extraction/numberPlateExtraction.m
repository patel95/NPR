...
f=imread('carplate1.jpg'); % Reading the number plate image.
imshow(f);
...
figure, imshow(g), title('greyscale');
...
figure, imshow(out1), title('after eroding');
...
H=bwmorph(F,'thin',3);
figure,imshow(H), title('after thinning...');

 H=imerode(H,strel('line',2,90));
figure,imshow(H), title('after eroding2...');
...
% Bounding boxes are acquired 00  
...
if ~isempty(r) % If succesfully indices of desired boxes are achieved.
    ...
    for v=1:length(r)
        ...
        if letter=='O' || letter=='0' % Since it wouldn't be easy to distinguish
            if v<3 || (v>4 && v<7)                      % between '0' and 'O' during the extraction of character
        ...
        else if letter=='Z' || letter=='2' % Since it wouldn't be easy to distinguish
            if v<3 || (v>4 && v<7)                      % between '0' and 'O' during the extraction of character
                letter='Z';              % in binary image. Using the characteristic of plates in Karachi
            else                         % that starting three characters are alphabets, this code will
                letter='2';              % easily decide whether it is '0' or 'O'. The condition for 'if'
            end   
        end
